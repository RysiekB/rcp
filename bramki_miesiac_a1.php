...
try 
{
    ...
    $uzytkownik = ltrim(rtrim($_POST['pracownik']));
    $NumerPrac = (int)$uzytkownik;
    $sql = "SET NOCOUNT ON; EXEC dbo.TA_Bramki_miesiac ?";
...
<html lang="pl">
    ...
    <hr> 
        ...
        <h3>Wejścia i wyjścia (kołowrotek) - bieżący miesiąc</h3>

        <table>
            <thead>
				<tr><th>Lp</th><th>Imię</th><th>Nazwisko</th><th>Firma</th><th>Numer ID</th><th>Numer karty</th><th>Wejście</th><th>Wyjście</th></tr>
			</thead>
			<tbody>
				<?php
                    $lp = 1;
                    foreach($rekordy as $row)
                    {
                        if ($row['Numer ID'] == $uzytkownik)
                        {
                            ...
                            echo "<tr><td>$lp</td><td>{$row['Imie']}</td><td>{$row['Nazwisko']}</td><td>{$row['Firma']}</td><td>{$row['Numer ID']}</td><td>{$row['Karta']}</td><td>$wejscie</td><td>$wyjscie</td></tr>";
                            $lp++;
                    }    }
				?>
			</tbody>
