...
try 
{
    ...
    $sql = "SET NOCOUNT ON; EXEC dbo.TA_WeWy_rokP ?";
...
<html lang="pl">
    ...
    <hr> 
        ...
        <h3>Czas pracy - bieżący rok</h3>
        <table>
            <thead>
				<tr><th>Lp</th><th>Imię</th><th>Nazwisko</th><th>Numer ID</th><th>Numer karty</th><th>Wejście</th><th>Wyjście</th><th>Czas pracy</th></tr>
			</thead>
			<tbody>
				<?php
                    $lp = 1;
                    foreach($rekordy as $row)
                    {
                        if ($row['Numer ID'] == $uzytkownik)
                        {
                            ...
                            echo "<tr><td>$lp</td><td>{$row['Imie']}</td><td>{$row['Nazwisko']}</td><td>{$row['Numer ID']}</td><td>{$row['Karta']}</td><td>$wejscie</td><td>$wyjscie</td><td>{$row['Czas Pracy']}</td></tr>";
                            $lp++;
                        }    
                    }
				?>
