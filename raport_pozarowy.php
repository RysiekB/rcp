...
try 
{
    ...
    $sql = "SET NOCOUNT ON; EXEC dbo.TA_Raport_pozarowy";
...
<html lang="pl">
    ...
    <hr> 
        ...
        <?php
            ...
            case "U":
                echo '<center>';
                echo '<br /><h3 class="error">Nie masz uprawnień do tego raportu.</h3><br /><br />';
                echo '<a href="glowna.php">Powrót do strony głównej</a>';
                echo '</center>';
                exit();
                break;
        ...
        <div class="glowneokno">
            <h3><span style="color:red;">R A P O R T &nbsp;&nbsp; P O Ż A R O W Y - Wejścia dzisiaj</span></h3>
        <table>
            <thead>
				<tr><th>Lp</th><th>Nazwisko</th><th>Imię</th><th>Firma</th><th>Numer ID</th><th>Wejście</th></tr>
			</thead>
			<tbody>
				<?php
                    $lp = 1;
                    foreach($rekordy as $row)
                    {
                        ...
                        echo "<tr><td>$lp</td><td>{$row['Nazwisko']}</td><td>{$row['Imie']}</td><td>{$row['Firma']}</td><td>{$row['Numer ID']}</td><td>$wejscie</td></tr>";
                        $lp++;
                    }
				?>
			</tbody>
