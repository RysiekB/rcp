...
<html lang="pl">
    ...
    <hr> 
        ...
        <table>
            <thead>
				<tr><th>Lp</th><th>Firma</th><th>Numer ID</th><th>Numer karty</th><th>Wejście</th><th>Wyjście</th></tr>
			</thead>
			<tbody>
				<?php
                    $lp = 1;
                    foreach($rekordy as $row)
                    {
                        if ($row['Numer ID'] == $uzytkownik)
                        {
                            ...
                            echo "<tr><td>$lp</td><td>{$row['Firma']}</td><td>{$row['Numer ID']}</td><td>{$row['Karta']}</td><td>$wejscie</td><td>$wyjscie</td></tr>";
                            $lp++;
                    }    }
				?>
			</tbody>
